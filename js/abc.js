...
function ABC(canvas) {
    ...
    this.tail_index_buffer = ABC.create_index_array(gl, length);
    this.tail_element_buffer = ABC.create_element_array(gl, length);
    ...
    ABC.fetch(shaders, function(project, tail_v, tail_f, head_v, head_f) {
        this.programs.tail = ABC.compile(gl, project + tail_v, tail_f);
        this.programs.head = ABC.compile(gl, project + head_v, head_f);
 ...
 */
ABC.fetch = function(urls, callback) {
    var results = [];
 ...
 * @returns {Object}
 */
ABC.compile = function(gl, vert, frag) {
 ...
 * @returns {WebGLBuffer}
 */
ABC.create_element_array = function(gl, length) {
 ...
 * @returns {WebGLBuffer}
 */
ABC.create_index_array = function(gl, length) {
...
/**
 * Update s to the next ABC state using RK4.
 ...
 * @returns {undefined}
 */
ABC.abc = function(s, dt, A, B, C, k) {
    function dx(x, y, z) { return B * Math.cos(k*y) + C * Math.sin(k*z); }
    function dy(x, y, z) { return C * Math.cos(k*z) + A * Math.sin(k*x); }
    function dz(x, y, z) { return A * Math.cos(k*x) + B * Math.sin(k*y); }
 ...
 * @returns {undefined}
 */
ABC.add_noise = function(s, dt, noise) {
 ...
 * @param {number} b
 */
ABC.prototype._update = function(a, b) {
    var gl = this.gl;
    var length = this.display._length;
    ...
    if (!this.params.paused) {
        ...
        for (var s = 0; s < this.params.steps_per_frame; s++) {
            ...
            for (var i = 0; i < this.solutions.length; i++)  {
                ABC.abc(this.solutions[i], dt, a, b, c, k);
                if (noise > 0) {
                    ABC.add_noise(this.solutions[i], dt, noise);
                }
 ...
 */
ABC.prototype._grow_buffers = function() {
    function next2(x) {
 ...
 * @returns {ABC} this
 */
ABC.prototype.add_batch = function(ss) {
    // var gl = this.gl;
    // var length = this.display._length;
 ...
 * @returns {ABC} this
 */
ABC.prototype._trim = function(length) {
    ...
    this.tail_index_buffer = ABC.create_index_array(this.gl, length);
    this.tail_element_buffer = ABC.create_element_array(this.gl, length);
    this._update(0, length - 1);
    return this;
};
...
Object.defineProperty(ABC.prototype, 'length', {
    get: function() {
 ...
 * @returns {ABC}
 */
ABC.run = function(canvas) {
    var abc = new ABC(canvas);
    for (var i = 0; i < 13; i++)
        abc.add(ABC.generate());
    function go() {
        abc.step();
        abc.draw();
